...
import { getCompletedAttemptsFromDb } from '@/lib/turso';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const attempts = await getCompletedAttemptsFromDb();
    ...
    console.error('Fetch completed attempts error:', error);
    return NextResponse.json(
      { success: false, error: error?.message || 'Failed to fetch completed attempts' },
      { status: 500 }
    );
  }
}
